...
`main` 分支只要 `src/**` 有變動，`.github/workflows/deploy-backend.yml` 會自動
`clasp push` + `clasp deploy` 到正式 `/exec`，不需再手動兩步。也可到 Actions 頁面
手動觸發（workflow_dispatch）。

需在 **GitHub 儲存庫 → Settings → Secrets and variables → Actions** 設定三個 Secret：

| Secret | 說明 | 如何取得 |
|---|---|---|
| `CLASPRC_JSON` | clasp 登入憑證整份 JSON | 本機 `clasp login` 後，`cat ~/.clasprc.json` 的完整內容 |
| `CLASP_SCRIPT_ID` | GAS 專案的 Script ID | 本機 `.clasp.json` 裡的 `scriptId`，或線上編輯器網址 |
| `CLASP_DEPLOYMENT_ID` | 正式 `/exec` 部署的 deployment ID | 本機 `clasp deployments` 列出，取 `@HEAD` 以外的正式部署 ID |

> **注意：** `CLASPRC_JSON` 含 refresh token，等同你的 GAS 操作權限，務必只放在 Secret，
> 絕不進版控。token 若過期需重新 `clasp login` 並更新此 Secret。
